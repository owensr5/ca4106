...
def firestore():
    ...
    # Ensure no moviess exist before running the tests. This typically helps if
    # tests somehow left the database in a bad state.
    delete_all_moviess(firestore)

    yield firestore

    # Delete all moviess that we created during tests.
    delete_all_moviess(firestore)


def delete_all_moviess(firestore):
    while True:
        moviess, _ = firestore.next_page(limit=50)
        if not moviess:
            break
        for movies in moviess:
            firestore.delete(movies['id'])


def test_list(app, firestore):
    for i in range(1, 12):
        firestore.create({'title': u'movies {0}'.format(i)})
    ...
    assert 'movies 1' in body, "Should show moviess"
    assert len(re.findall('<h4>movies', body)) <= 10, (
        "Should not show more than 10 moviess")
...
def test_add(app):
    ...
    with app.test_client() as c:
        rv = c.post('moviess/add', data=data, follow_redirects=True)

    assert rv.status == '200 OK'
    body = rv.data.decode('utf-8')
    assert 'Test movies' in body
