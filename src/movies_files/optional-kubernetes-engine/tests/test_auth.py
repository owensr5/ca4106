...
class TestAuth(object):
    ...
    def test_mine(self, model, client_with_credentials):
        # Create two moviess, one created by the logged in user and one
        ...
        # Check the "My moviess" page and make sure only one of the moviess
        # appears.
        with client_with_credentials() as c:
            rv = c.get('/moviess/mine')

        assert rv.status == '200 OK'
        body = rv.data.decode('utf-8')
        assert 'movies 1' in body
        assert 'movies 2' not in body
    ...
    def test_request_user_info(self, HttpMock):
        ...
        moviesshelf._request_user_info(credentials)
