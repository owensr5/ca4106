...
def test_end_to_end():
    ...
    To run locally, start both main.py and psq_worker main.moviess_queue and
    run this file.
    ...
    response = requests.post(base_url + '/moviess/add', data=movies_data)

    # There was a 302, so get the movies's URL from the redirect.
    movies_url = response.request.url
    movies_id = movies_url.rsplit('/', 1).pop()
    ...
    def check_for_updated_data():
        # Check that the movies's information was updated.
        response = requests.get(movies_url)
        assert response.status_code == 200

        soup = BeautifulSoup(response.text, 'html.parser')

        title = soup.find('h4', 'movies-title').contents[0].strip()
        assert re.search(r'A Confederacy of Dunces', title, re.I)

        author = soup.find('h5', 'movies-author').string
        assert re.search(r'John Kennedy Toole', author, re.I)

        description = soup.find('p', 'movies-description').string
        assert re.search(r'Ignatius', description, re.I)

        image_src = soup.find('img', 'movies-image')['src']
    ...
    try:
        ...
        # Delete the movies we created.
        requests.get(base_url + '/moviess/{}/delete'.format(movies_id))
