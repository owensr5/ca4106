...
def model(monkeypatch, app):
    ...
    """
    model = moviesshelf.get_model()

    # Ensure no moviess exist before running. This typically helps if tests
    # somehow left the database in a bad state.
    delete_all_moviess(model)
...
def delete_all_moviess(model):
    while True:
        moviess, _ = model.list(limit=50)
        if not moviess:
            break
        for movies in moviess:
            model.delete(movies['id'])
